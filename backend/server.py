...
async def download_installer_zip():
    ...
    path = "/app/electron/GadirTV-Installer-v1.6.zip"
    if not os.path.exists(path):
        raise HTTPException(404, "installer zip not built yet")
    return FileResponse(path, filename="GadirTV-Installer-v1.6.zip", media_type="application/zip", headers=NO_CACHE_HEADERS)


@app.get("/api/download/portable")
async def download_portable():
    path = "/app/electron/GadirTV-Portable-v1.6.zip"
    if not os.path.exists(path):
        raise HTTPException(404, "portable zip not built yet")
    return FileResponse(path, filename="GadirTV-Portable-v1.6.zip", media_type="application/zip", headers=NO_CACHE_HEADERS)
...
async def download_encrypted():
    ...
    path = "/app/electron/GadirTV-Portable-v1.6-encrypted.zip"
    if not os.path.exists(path):
        raise HTTPException(404, "encrypted zip not built yet")
    return FileResponse(path, filename="GadirTV-Portable-v1.6-encrypted.zip", media_type="application/zip", headers=NO_CACHE_HEADERS)
